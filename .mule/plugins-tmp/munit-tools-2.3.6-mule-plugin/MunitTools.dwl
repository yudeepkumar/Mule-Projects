...
fun greaterThan(value: Comparable): Matcher = {
    "type" : "greaterThan",
    expected: [(actual) -> do {
        if(actual > value) createMatch() as MatcherDiff
        else createDiff(" to be greater than $(value)", actual, "(root)") as MatcherDiff
    }]
} as Matcher
...
fun lessThan(value: Comparable): Matcher = {
    ...
    expected: [(actual) -> do {
        if(actual < value) createMatch() as MatcherDiff
        else createDiff(" to be less than $(value)", actual, "(root)") as MatcherDiff
    }]
} as Matcher
